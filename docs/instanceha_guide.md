...
**Important**: Watchdog cannot be the sole fencing agent. It must be used as a fallback after a BMC-based agent (e.g. redfish, ipmi). Startup validation rejects configurations where watchdog is the only agent.
...
- `watchdog` cannot be the sole fencing agent (startup validation rejects this)
