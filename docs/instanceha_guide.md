...
**Example:** Nova reports 40 hosts as down across the region. Only 8 are in evacuable aggregates and carry evacuable workloads. With `TAGGED_AGGREGATES: true` and `THRESHOLD: 50`, the agent calculates `8 / total_evacuable × 100` — not `40 / total_active × 100`. A datacenter-wide outage that mostly affects non-evacuable or untagged hosts may **not** trip the global threshold, even though many physical hosts are unhealthy.
...
After 3 consecutive cliff detections (3 × 45s = ~135s with default settings), the cliff detector expires — it assumes the loss is genuine, not transient. At this point, remaining defenses take over:
...
| `HEARTBEAT_TIMEOUT` | 120s | How long a heartbeat is considered fresh. Higher values tolerate longer network blips but delay detection of real failures. |

#### Tuning by cluster size

The defaults are designed for small-to-medium clusters (up to ~50 nodes). As the cluster grows, the percentage-based thresholds need adjustment because a single failure domain (rack, switch) represents a smaller fraction of the total.
...
HEARTBEAT_CLIFF_MAX_CYCLES: "5"      # 5 × 120s POLL = 10 min buffer
