...
var _ = Describe("RabbitMQ Controller", func() {
	...
	When("RabbitMQ mirrored queues and version compatibility", func() {
		...
		When("RabbitMQ 4.2 with Mirrored queues", func() {
			...
			It("should skip mirrored queue policy on RabbitMQ 4.2+", func() {
				// Trigger a reconciliation by simulating cluster ready again
				// This ensures the controller processes the 4.2 version and clears Status.QueueType
				SimulateRabbitMQClusterReady(rabbitmqName)

				// Trigger another reconciliation to ensure status is updated
				// by updating a harmless field
				Eventually(func(g Gomega) {
					instance := GetRabbitMQ(rabbitmqName)
					if instance.Annotations == nil {
						instance.Annotations = make(map[string]string)
					}
					if instance.Annotations["test-trigger"] == "" {
						instance.Annotations["test-trigger"] = "reconcile"
				...
				// Verify webhook converted Mirrored to Quorum for RabbitMQ 4.2+
				// The webhook's DefaultForUpdate() enforces Quorum for 4.2+
				Eventually(func(g Gomega) {
					instance := GetRabbitMQ(rabbitmqName)
					g.Expect(instance.Status.CurrentVersion).To(Equal("4.2"))
					// Webhook converts Mirrored → Quorum, so Spec.QueueType is Quorum
					g.Expect(instance.Spec.QueueType).ToNot(BeNil())
					g.Expect(*instance.Spec.QueueType).To(Equal("Quorum"),
						"Webhook should override Mirrored to Quorum on RabbitMQ 4.2+")
					// Status should match the converted spec
					g.Expect(instance.Status.QueueType).To(Equal("Quorum"))
				}, timeout, interval).Should(Succeed())
	...
	When("RabbitMQ deletion with preserved user and backup secret", func() {
		It("should set owner references to RabbitMq CR and garbage collect resources after finalizer removal", func() {
			// Create RabbitMQ instance
			spec := GetDefaultRabbitMQSpec()
			_ = CreateRabbitMQ(rabbitmqName, spec)

			// Wait for it to be ready
			SimulateRabbitMQClusterReady(rabbitmqName)

			// Capture the CR UID for garbage collection simulation later
			var crUID types.UID
			Eventually(func(g Gomega) {
				instance := GetRabbitMQ(rabbitmqName)
			...
			// Manually add finalizer to RabbitMQ CR
			// In production, lib-common helper adds this, but in test we do it explicitly
			// Format: "openstack.org/<kind>" (lowercase)
			Eventually(func(g Gomega) {
				instance := GetRabbitMQ(rabbitmqName)
			...
			Eventually(func(g Gomega) {
				instance := GetRabbitMQ(rabbitmqName)
				g.Expect(instance.Finalizers).ToNot(BeEmpty(), "RabbitMQ CR should have a finalizer")
			}, timeout, interval).Should(Succeed())

			// Simulate a scenario where backup user and secret exist (as during upgrade)
			// Create backup secret with owner reference (simulating what happens during storage wipe)
			backupSecretName := types.NamespacedName{
				Name:      rabbitmqName.Name + "-default-user-backup",
				Namespace: namespace,
			}

			// Get the RabbitMq CR to use its UID for owner reference
			instance := GetRabbitMQ(rabbitmqName)
			backupSecret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
			...
			// Create preserved RabbitMQUser (simulating what happens after upgrade)
			preservedUserName := types.NamespacedName{
				Name:      rabbitmqName.Name + "-default-user-preserved",
				Namespace: namespace,
			}
			preservedUserSpec := map[string]any{
				"rabbitmqClusterName": rabbitmqName.Name,
				"username":            "test_user",
				"secret":              backupSecretName.Name,
			...
			_ = CreateRabbitMQUser(preservedUserName, preservedUserSpec)

			// Simulate user being ready (in real scenario, RabbitMQUser controller does this)
			SimulateRabbitMQUserReady(preservedUserName, "/")

			// Verify preserved user exists and is ready with finalizer
			// The finalizer is critical - without it, deletion would be immediate
			Eventually(func(g Gomega) {
				user := GetRabbitMQUser(preservedUserName)
				g.Expect(user).ToNot(BeNil())
				g.Expect(user.Status.Conditions.IsTrue(condition.ReadyCondition)).To(BeTrue())
				// Wait for finalizer to be added by controller
				g.Expect(user.Finalizers).To(ContainElement("rabbitmquser.openstack.org/finalizer"))
			}, timeout, interval).Should(Succeed())

			// Verify ConfigMaps exist and have owner references to RabbitMq CR
			Eventually(func(g Gomega) {
				configMapName := types.NamespacedName{
					Name:      rabbitmqName.Name + "-config-data",
					Namespace: namespace,
				...
				cm := &corev1.ConfigMap{}
				err := k8sClient.Get(ctx, configMapName, cm)
				g.Expect(err).ToNot(HaveOccurred())
				// ConfigMaps should have owner reference to RabbitMq CR for automatic garbage collection
				g.Expect(cm.OwnerReferences).ToNot(BeEmpty())
				g.Expect(cm.OwnerReferences[0].Kind).To(Equal("RabbitMq"))
				g.Expect(cm.OwnerReferences[0].Name).To(Equal(rabbitmqName.Name))
			}, timeout, interval).Should(Succeed())

			// Trigger deletion of RabbitMQ CR
			Eventually(func(g Gomega) {
				instance := &rabbitmqv1.RabbitMq{}
				err := k8sClient.Get(ctx, rabbitmqName, instance)
				if err != nil {
			...
			// 1. Wait for RabbitMQ controller to mark preserved user for deletion
			// The RabbitMQ reconcileDelete() should delete the user
			Eventually(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				err := k8sClient.Get(ctx, preservedUserName, user)
				if err != nil {
					// User already fully deleted - that's fine too
					g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue())
					return
				}
				// User still exists - should have deletion timestamp from RabbitMQ controller
				g.Expect(user.DeletionTimestamp).ToNot(BeNil(),
					"RabbitMQUser should be marked for deletion by RabbitMQ controller")
			}, timeout, interval).Should(Succeed())

			// 2. Simulate RabbitMQUser controller completing its deletion
			// In real scenario, the RabbitMQUser controller removes its finalizer after cleanup
			// Since we can't connect to RabbitMQ in test, manually remove finalizer
			Eventually(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				err := k8sClient.Get(ctx, preservedUserName, user)
				if k8s_errors.IsNotFound(err) {
				...
				if user.DeletionTimestamp != nil && len(user.Finalizers) > 0 {
					// Remove finalizer to simulate successful cleanup
			...
			// 3. Wait for user to be fully deleted
			Eventually(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				err := k8sClient.Get(ctx, preservedUserName, user)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue(), "RabbitMQUser should be fully deleted")
			}, timeout, interval).Should(Succeed())

			// 4. Verify RabbitMQCluster is deleted
			// The RabbitMQ controller deletes it in reconcileDelete()
			Eventually(func(g Gomega) {
				cluster := &rabbitmqclusterv2.RabbitmqCluster{}
				err := k8sClient.Get(ctx, rabbitmqName, cluster)
			...
			// Note: In envtest, there are no real pods, so waitForPodsTermination() completes immediately
			// The controller removes the finalizer right away

			// 5. Verify RabbitMQ CR is fully deleted after finalizer removal
			// (May take a bit longer in test environment for garbage collection)
			Eventually(func(g Gomega) {
				instance := &rabbitmqv1.RabbitMq{}
				err := k8sClient.Get(ctx, rabbitmqName, instance)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue(), "RabbitMQ CR should be fully deleted")
			}, timeout*2, interval).Should(Succeed())

			// Simulate garbage collection (envtest doesn't have a real garbage collector)
			// This deletes resources with owner references to the deleted CR
			SimulateGarbageCollection(crUID, namespace)

			// 6. Verify ConfigMaps are garbage collected after CR deletion
			// In production, Kubernetes garbage collection handles this automatically
			Eventually(func(g Gomega) {
				configMapName := types.NamespacedName{
					Name:      rabbitmqName.Name + "-config-data",
					Namespace: namespace,
				...
				cm := &corev1.ConfigMap{}
				err := k8sClient.Get(ctx, configMapName, cm)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue(), "ConfigMap should be garbage collected after CR deletion")
			}, timeout, interval).Should(Succeed())

			// 7. Verify backup secret is garbage collected after CR deletion
			// Secret has owner reference to RabbitMq CR for automatic cleanup
			Eventually(func(g Gomega) {
				secret := &corev1.Secret{}
				err := k8sClient.Get(ctx, backupSecretName, secret)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue(), "Backup secret should be garbage collected after CR deletion")
			}, timeout, interval).Should(Succeed())
		})

		It("should not delete backup secret before RabbitMQUser is deleted", func() {
			// Create RabbitMQ instance
			spec := GetDefaultRabbitMQSpec()
			_ = CreateRabbitMQ(rabbitmqName, spec)
			...
			// Add finalizer to RabbitMQ CR (format: "openstack.org/<kind>" lowercase)
			Eventually(func(g Gomega) {
				instance := GetRabbitMQ(rabbitmqName)
				if len(instance.Finalizers) == 0 {
					instance.Finalizers = []string{"openstack.org/rabbitmq"}
			...
			// Create preserved RabbitMQUser that references the backup secret
			preservedUserName := types.NamespacedName{
				Name:      rabbitmqName.Name + "-default-user-preserved",
				Namespace: namespace,
			}
			...
			CreateRabbitMQUser(preservedUserName, preservedUserSpec)

			// Simulate user being ready
			SimulateRabbitMQUserReady(preservedUserName, "/")

			// Verify user is ready with finalizer
			Eventually(func(g Gomega) {
				user := GetRabbitMQUser(preservedUserName)
				g.Expect(user.Status.Conditions.IsTrue(condition.ReadyCondition)).To(BeTrue())
				g.Expect(user.Finalizers).To(ContainElement("rabbitmquser.openstack.org/finalizer"))
			...
			// Verify that while RabbitMQUser still exists, the backup secret is NOT deleted
			Consistently(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				userErr := k8sClient.Get(ctx, preservedUserName, user)

				secret := &corev1.Secret{}
				secretErr := k8sClient.Get(ctx, backupSecretName, secret)

				// If user still exists, secret should still exist
				if userErr == nil {
					g.Expect(secretErr).ToNot(HaveOccurred(), "Backup secret should exist while RabbitMQUser exists")
				}
			}, "5s", "1s").Should(Succeed())

			// Clean up: Simulate RabbitMQUser controller completing deletion
			Eventually(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				err := k8sClient.Get(ctx, preservedUserName, user)
				if k8s_errors.IsNotFound(err) {
			...
			// Wait for user to be fully deleted
			Eventually(func(g Gomega) {
				user := &rabbitmqv1.RabbitMQUser{}
				err := k8sClient.Get(ctx, preservedUserName, user)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue())
			...
			// Wait for full cleanup (may take longer for garbage collection in test)
			Eventually(func(g Gomega) {
				instance := &rabbitmqv1.RabbitMq{}
				err := k8sClient.Get(ctx, rabbitmqName, instance)
				g.Expect(k8s_errors.IsNotFound(err)).To(BeTrue(), "RabbitMQ CR should be fully deleted")
