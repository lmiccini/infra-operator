#!/usr/bin/env python3
"""Tests for the InstanceHA LLM integration."""
...
from instanceha_ai.engine import (
    LLMEngine, LLMResponse, LocalEngine, RemoteEngine, ToolCall, create_engine,
)
from instanceha_ai.prompts import build_system_prompt, build_context_message
from instanceha_ai.context import summarize_cluster, summarize_recent_events, build_context
from instanceha_ai.agent import Agent, AgentResponse
...
class TestRemoteEngine(unittest.TestCase):
    def test_model_info(self):
        engine = RemoteEngine(endpoint="http://localhost:8000", model="llama-3.1-8b")
        info = engine.model_info()
    ...
    def test_chat_success(self, mock_urlopen):
        ...
        mock_response.read.return_value = json.dumps({
            "choices": [{"message": {"content": "Healthy.", "role": "assistant"},
                         "finish_reason": "stop"}],
        ...
        resp = engine.chat([{"role": "user", "content": "status?"}])
    ...
    def test_chat_with_tool_call(self, mock_urlopen):
        ...
        mock_response.read.return_value = json.dumps({
            "choices": [{"message": {
                ...
                "tool_calls": [{"id": "call_123", "function": {
                    "name": "get_compute_services", "arguments": "{}"}}],
        ...
        engine = RemoteEngine(endpoint="http://localhost:8000", model="test")
        resp = engine.chat(
            [{"role": "user", "content": "show computes"}],
...
class TestCreateEngine(unittest.TestCase):
    def test_no_config(self):
        self.assertIsNone(create_engine({}))
    ...
    def test_local_engine_missing_file(self):
        engine = create_engine({"model_path": "/nonexistent/model.gguf"})
...
class TestContext(unittest.TestCase):
    def _make_service(self, host, state, status, forced_down=False, disabled_reason=""):
        return Mock(host=host, state=state, status=status,
...
class TestAgent(unittest.TestCase):
    ...
    def test_simple_text_response(self):
        engine = MockEngine(responses=[LLMResponse(content="Cluster is healthy.")])
        agent = Agent(engine, self.approval_manager, self.mock_conn, self.mock_service)
        result = agent.query("How is the cluster?")
        self.assertEqual(result.message, "Cluster is healthy.")
    ...
    def test_tool_call_then_response(self):
        ...
        engine = MockEngine(responses=[
            LLMResponse(
                ...
                tool_calls=[ToolCall(name="get_compute_services", arguments={}, call_id="call_1")],
                finish_reason="tool_calls",
            ),
            LLMResponse(content="1 compute service (h1), up and enabled."),
        ])
        agent = Agent(engine, self.approval_manager, self.mock_conn, self.mock_service)
    ...
    def test_tool_call_needing_approval(self):
        engine = MockEngine(responses=[
            LLMResponse(
                content="I'll fence compute-1.",
                tool_calls=[ToolCall(name="fence_host",
    ...
    def test_max_iterations_reached(self):
        responses = [
            LLMResponse(
                ...
                tool_calls=[ToolCall(name="get_config", arguments={}, call_id=f"call_{i}")],
                finish_reason="tool_calls",
            ) for i in range(10)
        ]
    ...
    def test_conversation_history(self):
        ...
        agent.query("Q2")
        self.assertEqual(len(agent._history), 4)
