...
class TestFenceWatchdog(unittest.TestCase):
    ...
    @patch('instanceha._interruptible_sleep')
    @patch('instanceha.time.monotonic')
    def test_shutdown_during_wait_returns_false(self, mock_time, mock_sleep):
        """Returns False when shutdown fires during the wait period."""
        mock_time.side_effect = [200.0]
        svc = self._make_service(listener_start=100.0, watchdog_timeout=60, last_hb=155.0)
        svc.shutdown_event.is_set = Mock(return_value=True)
        result = instanceha._fence_watchdog("compute-0", "off", {}, svc)
        self.assertFalse(result)
        self.assertNotIn('compute-0', svc.watchdog_fenced_hosts)

    @patch('instanceha.time.monotonic', return_value=200.0)
    def test_timeout_clamped_to_max(self, mock_time):
        """Timeout is clamped to maximum of 300 seconds."""
        svc = self._make_service(listener_start=100.0, watchdog_timeout=60, last_hb=0)
        result = instanceha._fence_watchdog("compute-0", "off", {'timeout': 500}, svc)
        # Silence is 100s (200-100 listener start), clamped timeout is 300s.
        # 100s < 300s, so enters wait path. But monotonic stays at 200.0,
        # so re-check still shows 100s < 300s -> fails.
        self.assertFalse(result)

    def test_validate_config_rejects_watchdog_sole_agent(self):
        """ConfigManager._validate_config raises ConfigurationError for watchdog as sole agent."""
        config = instanceha.ConfigManager()
        config.fencing = {'compute-0': {'agent': 'watchdog'}}
        with self.assertRaises(instanceha.ConfigurationError) as cm:
            config._validate_config()
        self.assertIn('sole', str(cm.exception).lower())
