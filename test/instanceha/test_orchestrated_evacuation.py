...
- _has_orchestration_metadata: detecting orchestration metadata on servers
...
- ORCHESTRATED_RESTART config option
"""
...
# ============================================================================
# _has_orchestration_metadata tests
# ============================================================================

class TestHasOrchestrationMetadata(unittest.TestCase):

    def test_no_metadata_returns_false(self):
        servers = [_make_server('s1'), _make_server('s2')]
        self.assertFalse(instanceha._has_orchestration_metadata(servers))

    def test_priority_metadata_returns_true(self):
        servers = [
            _make_server('s1'),
            _make_server('s2', metadata={'instanceha:restart_priority': '100'}),
        ]
        self.assertTrue(instanceha._has_orchestration_metadata(servers))

    def test_group_metadata_returns_true(self):
        servers = [
            _make_server('s1', metadata={'instanceha:restart_group': 'db'}),
        ]
        self.assertTrue(instanceha._has_orchestration_metadata(servers))

    def test_empty_list_returns_false(self):
        self.assertFalse(instanceha._has_orchestration_metadata([]))

    def test_unrelated_metadata_returns_false(self):
        servers = [_make_server('s1', metadata={'some_key': 'value'})]
        self.assertFalse(instanceha._has_orchestration_metadata(servers))
...
class TestHostEvacuateOrchestration(unittest.TestCase):
    ...
    @patch('instanceha._get_evacuable_servers')
    @patch('instanceha.time.sleep')
    def test_auto_detect_metadata(self, mock_sleep, mock_get, mock_trad, mock_smart, mock_orch):
