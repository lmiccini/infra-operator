    ...
    def _validate_config(self) -> None:
        ...
        if self.get_config_value('CHECK_HEARTBEAT'):
            ...
            if self.get_config_value('HEARTBEAT_TIMEOUT') <= self.get_config_value('WATCHDOG_TIMEOUT'):
                logging.warning(
                    'HEARTBEAT_TIMEOUT (%ds) is <= WATCHDOG_TIMEOUT (%ds) -- watchdog '
                    'fencing may trigger before heartbeat absence is confirmed',
                    self.get_config_value('HEARTBEAT_TIMEOUT'), self.get_config_value('WATCHDOG_TIMEOUT'))
...
class InstanceHAService:
    ...
    def __init__(self, config_manager: ConfigManager, cloud_client: Optional[OpenStackClient] = None):
        ...
        # Protected by heartbeat_lock -- updated by _fence_watchdog concurrently
...
def _host_enable(connection, nova_service, reenable: bool = False, service=None) -> bool:
    ...
    if reenable:
        ...
        if 'watchdog' in (getattr(nova_service, 'disabled_reason', '') or ''):
            try:
                connection.services.disable_log_reason(nova_service.id, _make_disable_reason(DISABLED_REASON_EVACUATION_COMPLETE))
                if service:
                    hostname = _extract_hostname(nova_service.host)
                ...
                logging.warning('Failed to clean up watchdog marker for %s: %s', nova_service.host, e)
        return True
...
def _bmh_fence(token, namespace, host, action, service):
    ...
    try:
        ...
        if e.response is not None and e.response.status_code in [401, 403]:
            logging.error("BMH fencing failed for %s: authentication error (non-retriable)", host)
            return None
...
def _execute_ipmi_fence(host, action, fencing_data, timeout, shutdown_event=None):
    ...
    try:
        ...
        if any(kw in stderr for kw in ('unable to establish', 'authorization',
                                        'password', 'privilege', 'authentication')):
            logging.error("IPMI %s for %s: authentication/authorization error -- "
                         "don't retry, don't fall back", action, host)
            return None
...
def _host_fence(host, action, service):
    ...
    Returns the agent name (str, truthy) on success, False on failure.
    """
    if not host:
    ...
    try:
        ...
        agents = _normalize_fencing_config(fencing_data)
        if not agents:
        ...
        for i, agent_config in enumerate(agents):
            if not isinstance(agent_config, dict) or 'agent' not in agent_config:
            ...
            if len(agents) > 1:
                logging.info("Trying fencing agent '%s' for %s (%d/%d)",
                            agent_name, host, i + 1, len(agents))
            else:
                logging.debug("Using fencing agent '%s' for %s", agent_name, host)
            result = _execute_fence_operation(host, action, agent_config, service)
...
def _post_evacuation_recovery(conn, failed_service, service, resume=False):
    ...
    try:
        # Step 1: Power on the host (skip if kdump or watchdog fenced)
        if kdump_fenced:
        ...
        try:
            ...
            if not resume:
                ...
                if not fence_agent:
                    ...
                    return False
                if fence_agent == 'watchdog':
                    _emit_k8s_event(host_name, 'FencingSucceeded',
                                    'Host confirmed dead by watchdog (passive, no BMC power-off)')
                else:
                    _emit_k8s_event(host_name, 'FencingSucceeded',
                                    f'Host fenced successfully (agent: {fence_agent})')
...
def _process_reenabling(conn, service, to_reenable) -> None:
    ...
    for svc in to_reenable:
        try:
            ...
            # Watchdog-fenced hosts don't need a delay -- the host is already silent.
