...
// Integration tests for the storage wipe workflow:
var _ = Describe("Storage Upgrade Flow Integration", func() {
	Context("when upgrading from RabbitMQ 3.9 to 4.2", func() {
		...
		It("should create new cluster with storage-wipe-needed annotation", func() {
			Skip("Integration test - requires full cluster")
		})
...
var _ = Describe("Storage Cleanup Approach", func() {
	It("documents that init container wipes data before RabbitMQ starts", func() {
		...
		// 2. Create new cluster with annotation: storage-wipe-needed=true
		// 3. Init container runs BEFORE RabbitMQ container
		// 4. Init container executes: rm -rf /var/lib/rabbitmq/* and creates version marker
		// 5. RabbitMQ starts with clean slate
		// 6. Annotation is removed in next reconcile after cluster is ready
		//
		// This works with ANY storage backend (PVCs are reused):
	...
	It("documents advantages of init container approach", func() {
		// Advantages of init container data wipe:
		// 1. Much simpler - just rm -rf, no PV/PVC orchestration
		// 2. Faster - no waiting for PV deletion
		// 3. More reliable - works even with static PVs
		// 4. Storage-agnostic - works regardless of reclaim policy
		// 5. Reuses same PVCs - less resource churn
		// 6. Idempotent - version marker prevents duplicate wipes
		// 7. Observable - can see annotation status
		//
		// The version marker file prevents data wipe on pod restarts
	})

	It("documents the annotation-based control mechanism", func() {
		// Annotation: rabbitmq.openstack.org/storage-wipe-needed
		// Value: "true" (temporary, removed after cluster is ready)
		//
		// Lifecycle:
		// 1. UpgradePhase="WaitingForCluster" → add annotation="true"
		// 2. Cluster created → init container wipes data (if marker doesn't exist)
		// 3. Init container creates version-specific marker file
		// 4. Cluster becomes ready → annotation removed in next reconcile
		// 5. Future pod restarts → no wipe (marker file exists)
		// 6. Next upgrade → new marker file for new version
	})
})

var _ = Describe("Edge Cases", func() {
	It("handles pod restarts during upgrade", func() {
		// If a pod restarts while annotation="true":
...
var _ = Describe("Upgrade Safety", func() {
	It("documents the simplified upgrade safety measures", func() {
		...
		// 3. Recreate cluster with storage-wipe-needed=true annotation
