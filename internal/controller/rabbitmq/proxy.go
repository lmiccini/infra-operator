...
func (r *Reconciler) addProxySidecar(
	...
	// The proxy container will mount this volume to get the server cert/key for TLS termination.
...
func (r *Reconciler) configureRabbitMQBackendPort(
	...
	// Configure listeners in both AdvancedConfig (Erlang) and AdditionalConfig (Cuttlefish)
	// Both are needed because Cuttlefish format takes precedence for listener settings

	// 1. DON'T modify AdvancedConfig for listeners - let Cuttlefish format handle it
	// The AdvancedConfig and Cuttlefish configs can conflict, so we only use Cuttlefish
	// to avoid ambiguity

	// 2. Add to AdditionalConfig (Cuttlefish format) - this takes precedence for listeners
	// Use numbered listener (listeners.ssl.1) to override the default (listeners.ssl.default)
	// Bind SSL listener to localhost:5674 to avoid conflict with proxy on 0.0.0.0:5671
	// The proxy handles client TLS on 5671, RabbitMQ backend is on localhost:5673
...
listeners.ssl.1 = 127.0.0.1:5674
listeners.ssl.default = 127.0.0.1:5674
`, rabbitmqBackendPort)

	// Prepend to existing AdditionalConfig so our settings come first
