    ...
    def parse_table(data: bytes, offset: int) -> Tuple[dict, int]:
        ...
        while offset < table_end and offset < len(data):
            ...
            if value_type == 't':  # Boolean
                ...
                value = []
                while offset < arr_end and offset < len(data):
                    # Each array element has a type prefix
                    elem_type = chr(data[offset])
                    offset += 1
                    # Re-wrap as single-entry table parse for type dispatch
                    elem_data = bytes([len(b'_'), ord('_'), ord(elem_type)]) + data[offset:]
                    elem_table, _ = AMQPMethodParser.parse_table(
                        struct.pack('!I', len(elem_data)) + elem_data, 0)
                    if '_' in elem_table:
                        value.append(elem_table['_'])
                    offset = arr_end  # Skip to end (arrays are opaque for our purposes)
                    break
                # For arrays we can't fully parse, preserve as raw bytes for passthrough
                if not value:
                    value = data[offset:arr_end]
                    offset = arr_end
            ...
            if isinstance(value, bool):
                ...
                for elem in value:
                    if isinstance(elem, bool):
                        arr_bytes += b't' + bytes([1 if elem else 0])
                    elif isinstance(elem, int):
                        arr_bytes += b'I' + struct.pack('!i', elem)
                    elif isinstance(elem, str):
                        encoded_elem = elem.encode('utf-8')
                        arr_bytes += b'S' + struct.pack('!I', len(encoded_elem)) + encoded_elem
                    elif isinstance(elem, dict):
                        arr_bytes += b'F' + AMQPMethodParser.encode_table(elem)
                table_bytes += b'A' + struct.pack('!I', len(arr_bytes)) + arr_bytes
            elif value is None:
                table_bytes += b'V'
            else:
...
class ExchangeDeclareRewriter:
    ...
    def rewrite(payload: bytes) -> Optional[bytes]:
        ...
        _passive = bool(flags & 0x01)
        durable = bool(flags & 0x02)
        _auto_delete = bool(flags & 0x04)
        _internal = bool(flags & 0x08)
        _no_wait = bool(flags & 0x10)
        ...
        try:
            ...
            if self.backend_tls:
                if self.tls_ca:
                    ...
                    # Disable hostname check for localhost/pod-local backends
