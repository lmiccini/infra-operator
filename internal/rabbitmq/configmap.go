...
func GeneratePluginsConfigMap(r *rabbitmqv1.RabbitMq) *corev1.ConfigMap {
	...
	plugins = append(plugins, r.Spec.Config.AdditionalPlugins...)
...
func GenerateServerConfigMap(
	...
	if r.Spec.Config.EnvConfig != "" {
		data["rabbitmq-env.conf"] = r.Spec.Config.EnvConfig
	}
	...
	if r.Spec.Config.ErlangInetConfig != "" {
		data["erl_inetrc"] = r.Spec.Config.ErlangInetConfig
	} else if IPv6Enabled {
